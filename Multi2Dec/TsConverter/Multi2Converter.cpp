...
const UINT_PTR CMulti2Converter::GetSyncErrNum(void) const
{
	// 同期エラー数を返す
	return (m_pTsPacketSync)? m_pTsPacketSync->GetSyncErrNum() : 0UL;
}

const UINT_PTR CMulti2Converter::GetFormatErrNum(void) const
{
	// フォーマットエラー数を返す
	return (m_pTsPacketSync)? m_pTsPacketSync->GetFormatErrNum() : 0UL;
}

const UINT_PTR CMulti2Converter::GetTransportErrNum(void) const
{
	// トランスポートエラー数を返す
	return (m_pTsPacketSync)? m_pTsPacketSync->GetTransportErrNum() : 0UL;
}

const UINT_PTR CMulti2Converter::GetContinuityErrNum(const WORD wPID) const
{
	// 連続性エラー数を返す
	...
	if(pDecoder == dynamic_cast<IMediaDecoder *>(m_pFileReader)){
		switch(dwEventID){
			...
			case IFileReader::EID_READ_END :
				// コンバート終了
				SendConverterEvent(EID_CONV_END, reinterpret_cast<PVOID>(m_pFileReader->GetReadPos() * 1000 / m_pFileReader->GetFileLength()));
				return 0UL;
			
			case IFileReader::EID_POST_READ :
				// コンバート進捗
				SendConverterEvent(EID_CONV_PROGRESS, reinterpret_cast<PVOID>(m_pFileReader->GetReadPos() * 1000 / m_pFileReader->GetFileLength()));
				return 0UL;
			}
		}
