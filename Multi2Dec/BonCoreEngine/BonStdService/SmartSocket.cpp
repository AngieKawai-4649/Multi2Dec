...
const DWORD CSmartSocket::HostNameToIPAddr(LPCTSTR lpszHostName)
{
	...
	::WideCharToMultiByte(CP_OEMCP, WC_COMPOSITECHECK, lpszHostName, static_cast<int>(::_tcslen(lpszHostName)), szHostName, sizeof(szHostName), NULL, NULL);
#else
...
#ifdef _UNICODE
	if(lpszHostName)::MultiByteToWideChar(CP_OEMCP, MB_PRECOMPOSED, pHost->h_name, -1, lpszHostName, static_cast<int>(::strlen(pHost->h_name)));
#else
	if(lpszHostName)::lstrcpy(lpszHostName, pHost->h_name);
#endif

	return static_cast<DWORD>(::strlen(pHost->h_name));
}
...
{
	struct timeval TimeVal = {static_cast<long>(dwTimeout / 1000UL), static_cast<long>((dwTimeout % 1000UL) * 1000UL)};
	fd_set FdSet{};
