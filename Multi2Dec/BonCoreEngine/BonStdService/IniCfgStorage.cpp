...
const bool CIniCfgStorage::OpenStorage(LPCTSTR lpszPathName, const bool bCreate)
{
	...
	if(!::GetModuleFileName(NULL, m_szFileName, sizeof(m_szFileName)/sizeof(TCHAR) - 1))return false;
...
const DWORD CIniCfgStorage::GetSectionNum(void)
{
	...
	// セクション名を列挙する
	static TCHAR szSections[MAX_SECTNAME_LEN * MAX_SECTNAME_NUM];
	memset(szSections, '\0', sizeof(szSections));

	const DWORD dwRet = ::GetPrivateProfileSectionNames(szSections, sizeof(szSections) / sizeof(szSections[0]), m_szFileName);
	if(!dwRet || ((dwRet + 2UL) == static_cast<DWORD>((sizeof(szSections) / sizeof(szSections[0])))))return false;
...
const DWORD CIniCfgStorage::EnumSection(const DWORD dwIndex, LPTSTR lpszSection)
{
	...
	const DWORD dwRet = ::GetPrivateProfileSectionNames(szSections, sizeof(szSections) / sizeof(szSections[0]), m_szFileName);
	if(!dwRet || ((dwRet + 2UL) == static_cast<DWORD>((sizeof(szSections) / sizeof(szSections[0])))))return 0UL;
	...
	while(*lpszFindPos){
		...
		if(IsSubSectionPath(lpszFindPos)){
			if(dwSectNum == dwIndex){
				...
				return static_cast<DWORD>(::_tcslen(lpszFindPos));
				}
...
const DWORD CIniCfgStorage::GetItemNum(void)
{
	...
	// アイテムを列挙する
	static TCHAR szItems[MAX_VALNAME_LEN * MAX_VALNAME_NUM];
	...
	if(!dwRet || ((dwRet + 2UL) == static_cast<DWORD>((sizeof(szItems) / sizeof(szItems[0])))))return 0UL;
...
const DWORD CIniCfgStorage::GetTextItem(LPCTSTR lpszItem, LPTSTR lpszValue, LPCTSTR lpszDefault, const DWORD dwMaxLen)
{
	...
	static TCHAR szValue[MAX_TEXTVAL_LEN];
	...
	// 文字数を返す
	return static_cast<DWORD>(::_tcslen(szValue));
}
...
{
	//if(!m_szFileName[0] || lpszPath==NULL || lpszSection==NULL)::DebugBreak();
	if (!m_szFileName[0] || lpszPath == NULL || lpszSection == NULL)return;;
...
const bool CIniCfgStorage::IsSubSectionPath(LPCTSTR lpszPath) const
{
	...
	if (!m_szFileName[0] || !lpszPath)return false;
	...
	if(!::_tcscmp(m_szSectName, ROOT_SECTNAME)){
		...
		// サブセクション以下
		const DWORD dwParentLen = static_cast<DWORD>(::_tcslen(m_szSectName) + 1UL);
...
const bool CIniCfgStorage::IsItemExist(LPCTSTR lpszItem) const
{
	...
	if(!dwRet || ((dwRet + 2UL) == static_cast<DWORD>(sizeof(szItems) / sizeof(szItems[0]))))return false;
