...
CString CMulti2DecWinDlg::ToUnitedText(const ULONGLONG llValue)
{
	CString csReturn;
	DWORD dwValue;
	double lfValue;
...
CString CMulti2DecWinDlg::ToCommaText(const ULONGLONG llValue)
{
	CString csValue;
	CString csReturn;
...
void CMulti2DecWinDlg::UpdateProgress(const UINT_PTR dwProgress)
{
	CString csText;

	// ファイルリスト更新
	csText.Format(TEXT("%Iu%%"), dwProgress / 10UL);
	...
	// プログレスバー更新
	m_Progress.SetPos(static_cast<int>(dwProgress));
}
...
void CMulti2DecWinDlg::OutputLogFile(void)
{
	...
	// 入力ファイル情報
	csLog.AppendFormat(TEXT("Input File: %s\r\n"), m_FileList.GetItemText(m_dwSrcIndex, 0).GetBuffer());

	csLog.AppendFormat(TEXT("Length: %12s (%s Byte)\r\n\r\n"),
		ToUnitedText(m_pMulti2Converter->GetSrcFileLength()).GetBuffer(),
		ToCommaText(m_pMulti2Converter->GetSrcFileLength()).GetBuffer()
		);		
	
	// 出力ファイル情報
	if(m_bCfgOutput){
		csLog.AppendFormat(TEXT("Output File: %s\r\n"), m_csDstPath.GetBuffer());
		
		csLog.AppendFormat(TEXT("Length: %12s (%s Byte)\r\n\r\n"),
			ToUnitedText(m_pMulti2Converter->GetDstFileLength()).GetBuffer(),
			ToCommaText(m_pMulti2Converter->GetDstFileLength()).GetBuffer()
			);
		}
	...
	csLog.AppendFormat(TEXT("\r\nSync Error       : %12s\r\n"), ToCommaText(m_pMulti2Converter->GetSyncErrNum()).GetBuffer());
	csLog.AppendFormat(TEXT("Format Error     : %12s\r\n"), ToCommaText(m_pMulti2Converter->GetFormatErrNum()).GetBuffer());
	csLog.AppendFormat(TEXT("Transport Error  : %12s Packet\r\n"), ToCommaText(m_pMulti2Converter->GetTransportErrNum()).GetBuffer());
	csLog.AppendFormat(TEXT("Total Drop Error : %12s Packet\r\n"), ToCommaText(m_pMulti2Converter->GetContinuityErrNum()).GetBuffer());
	csLog.AppendFormat(TEXT("Total Scrambling : %12s Packet\r\n"), ToCommaText(m_pMulti2Converter->GetScramblePacketNum()).GetBuffer());
	csLog.AppendFormat(TEXT("\r\nTotal Input      : %12s Packet\r\n"), ToCommaText(m_pMulti2Converter->GetInputPacketNum()).GetBuffer());

	if(m_bCfgOutput){
		csLog.AppendFormat(TEXT("Total Output     : %12s Packet\r\n"), ToCommaText(m_pMulti2Converter->GetOutputPacketNum()).GetBuffer());
		}

	csLog.AppendFormat(TEXT("Packet Stride    : %12lu Byte\r\n"), m_pMulti2Converter->GetPacketStride());

	if(m_bCfgB25){
		csLog.AppendFormat(TEXT("\r\nECM Process      : %12s\r\n"), ToCommaText(m_pMulti2Converter->GetEcmProcessNum()).GetBuffer());
	
		if(m_bCfgEmm){
			csLog.AppendFormat(TEXT("EMM Process      : %12s\r\n"), ToCommaText(m_pMulti2Converter->GetEmmProcessNum()).GetBuffer());
			}
		}
	...
	char* szMbText = (char*)malloc(dwWriteSize + 1UL);
	if (szMbText != NULL) {
...
{
	const UINT_PTR dwEventID = (const UINT_PTR)wParam;
	const UINT_PTR pParam = (const UINT_PTR)lParam;
...
void CMulti2DecWinDlg::OnCmdListAdd()
{
	...
	static TCHAR szFileNameBuffer[10000] = { 0 };      // ファイル名を保存させる為のバッファ
