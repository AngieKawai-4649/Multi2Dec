...
// ITspacketSync
	...
	virtual const UINT_PTR GetSyncErrNum(void);
	virtual const UINT_PTR GetFormatErrNum(void);
	virtual const UINT_PTR GetTransportErrNum(void);
	virtual const UINT_PTR GetContinuityErrNum(const WORD wPID = TS_INVALID_PID);
...
// CTsPacketSync
	...
	UINT_PTR m_dwSyncErrNum;
	UINT_PTR m_dwFormatErrNum;
	UINT_PTR m_dwTransportErrNum;
	UINT_PTR m_dwContinuityErrNum;

	DWORD m_adwInputPacketNum[0x2000];
	DWORD m_adwOutputPacketNum[0x2000];
	DWORD m_adwContinuityErrNum[0x2000];

	ULONGLONG m_dwLastUpdateTime;
