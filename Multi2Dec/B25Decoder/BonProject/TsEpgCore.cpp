...
const DWORD CEpgItem::GetEventTitle(LPTSTR lpszDst) const
{
	...
	return static_cast<DWORD>(m_strEventTitle.length());
}
...
const DWORD CEpgItem::GetEventInfo(LPTSTR lpszDst) const
{
	...
	return static_cast<DWORD>(m_strEventInfo.length());
}
...
const DWORD CEpgItem::GetEventDetail(LPTSTR lpszDst) const
{
	...
	return static_cast<DWORD>(m_strEventDetail.length());
}
...
{
	// ジャンル数を返す
	return static_cast<DWORD>(m_GenreArray.size());
}
