...
const DWORD CSmartFile::ReadData(BYTE *pBuff, const DWORD dwLen)
{
	...
	// ファイル読み取り
	const DWORD dwReturn = static_cast<DWORD>(::_fread_nolock(pBuff, 1UL, dwLen, m_pFileStream));
...
const DWORD CSmartFile::WriteData(const BYTE *pBuff, const DWORD dwLen)
{
	...
	// ファイル書き込み
	const DWORD dwReturn = static_cast<DWORD>(::_fwrite_nolock(pBuff, 1UL, dwLen, m_pFileStream));
