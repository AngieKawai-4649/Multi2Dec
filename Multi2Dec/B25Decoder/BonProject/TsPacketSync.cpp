...
void CTsPacketSync::ResetStatistics(void)
{
	...
	m_dwLastUpdateTime = ::GetTickCount64();
...
const UINT_PTR CTsPacketSync::GetSyncErrNum(void)
{
	// 同期エラー数を返す(パケット境界が乱れるモード)
	return m_dwSyncErrNum;
}

const UINT_PTR CTsPacketSync::GetFormatErrNum(void)
{
	// フォーマットエラー数を返す(パケットヘッダが規格外のモード)
	return m_dwFormatErrNum;
}

const UINT_PTR CTsPacketSync::GetTransportErrNum(void)
{
	// トランスポートエラー数を返す(Transport Error Indicatorが立つモード)
	return m_dwTransportErrNum;
}

const UINT_PTR CTsPacketSync::GetContinuityErrNum(const WORD wPID)
{
			...
			if((dwSize - dwCurPos) >= (TS_PACKET_SIZE - dwCurSize)){
				...
				if(ParseTsPacket()){
					...
					if((m_dwFixStrideSize != m_dwCurStrideSize) && m_dwFixStrideSize){
						...
						SendDecoderEvent(EID_SYNC_ERR, reinterpret_cast<PVOID>(static_cast<intptr_t>(m_dwSyncErrNum)));
						}
...
const bool CTsPacketSync::ParseTsPacket(void)
{
	...
	switch(m_TsPacket.ParsePacket(m_abyContCounter)){
		...
		case ITsPacket::EC_FORMAT :
			...
			SendDecoderEvent(EID_FORMAT_ERR, reinterpret_cast<PVOID>(static_cast<intptr_t>(m_dwFormatErrNum)));
			return false;
		
		case ITsPacket::EC_TRANSPORT :
			// トランスポートエラー異常
			CLIPED_INCREMENT(m_dwTransportErrNum);
			SendDecoderEvent(EID_TRANSPORT_ERR, reinterpret_cast<PVOID>(static_cast<intptr_t>(m_dwTransportErrNum)));
			return false;
		...
		case ITsPacket::EC_CONTINUITY :
			...
			CLIPED_INCREMENT(m_dwCurInputPacket);
			SendDecoderEvent(EID_CONTINUITY_ERR, reinterpret_cast<PVOID>(static_cast<intptr_t>(m_dwContinuityErrNum)));
			return true;
...
{
	// ビットレート更新
	const ULONGLONG dwCurUpdateTime = ::GetTickCount64();
