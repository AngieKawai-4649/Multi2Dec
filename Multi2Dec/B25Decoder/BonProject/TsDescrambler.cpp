...
const UINT_PTR CTsDescrambler::GetDescramblingState(const WORD wProgramID)
{
	// PATテーブルを取得
...
void CTsDescrambler::OutputPacket(ITsPacket * const pPacket)
{
	...
	if(pPacket->IsScrambled()){
		...
		SendDecoderEvent(EID_CANT_DESCRAMBLE, reinterpret_cast<PVOID>(static_cast<intptr_t>(m_dwScramblePacketNum)));
...
CTsDescrambler::CEcmProcessor::CEcmProcessor(IBonObject *pOwner)
	...
	, m_dwLastRetryTime(::GetTickCount64() - BCAS_REFRESH_INTERVAL)
	, m_dwDescramblingState(IHalBcasCard::EC_NO_ERROR)
	, m_bIsEcmReceived(FALSE)
{
	// MULTI2デコーダを初期化
...
const bool CTsDescrambler::CEcmProcessor::OnTableUpdate(const IPsiSection *pNewSection, const IPsiSection *pLastSection)
{
	...
	if(!pKsData && (m_pHalBcasCard->GetEcmError() != IHalBcasCard::EC_NOT_CONTRACTED)){
		if((::GetTickCount64() - m_dwLastRetryTime) >= BCAS_REFRESH_INTERVAL){
			// 再初期化ガードインターバル経過なら			
			...
			// 最終リトライ時間更新
			m_dwLastRetryTime = ::GetTickCount64();
			}
		}
	...
	m_pTsDescrambler->SendDecoderEvent(EID_ECM_PROCESSED, reinterpret_cast<PVOID>(static_cast<intptr_t>(m_pHalBcasCard->GetEcmError())));
