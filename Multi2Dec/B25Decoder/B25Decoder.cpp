...
const DWORD CB25Decoder::GetDescramblingState(const WORD wProgramID)
{
	...
	// 指定したプログラムIDの復号状態を返す
	return static_cast<DWORD>(m_TsDescrambler.GetDescramblingState(wProgramID));
}
...
{
	// 同期エラー数を返す
	return static_cast<DWORD>(m_TsPacketSync.GetSyncErrNum());
}

const DWORD CB25Decoder::GetFormatErrNum(void)
{
	// フォーマットエラー数を返す
	return static_cast<DWORD>(m_TsPacketSync.GetFormatErrNum());
}

const DWORD CB25Decoder::GetTransportErrNum(void)
{
	// ビットエラー数を返す
	return static_cast<DWORD>(m_TsPacketSync.GetTransportErrNum());
}

const DWORD CB25Decoder::GetContinuityErrNum(const WORD wPID)
{
	// ドロップパケット数を返す
	return static_cast<DWORD>(m_TsPacketSync.GetContinuityErrNum(wPID));
}
